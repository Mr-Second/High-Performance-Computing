...
subroutine jacobi2d_sub(n, tol, maxiter, iter, dumax, nthreads)
    ...
    real(kind=8), dimension(:,:), allocatable :: u,uold,f,uprime
    ! two dimension array
    real(kind=8), dimension(:), allocatable :: x,y
    ! one dimension array
    real(kind=8) :: h,o1,o2,o3,o4,c1,c2,omega,itermin
    integer :: i,j,k
	...
	u = uprime
        do iter=1,maxiter
            ...
            do j=1,n
                do i=1,n
		    if (mod(i+j,2)==0) then
                        u(i,j) = c1*uold(i,j)+c2*(uold(i-1,j) + uold(i+1,j) + &
            ...
            do j=1,n
                do i=1,n
		    if (mod(i+j,2)==1) then
                        u(i,j) = c1*uold(i,j)+c2*(u(i-1,j) + u(i+1,j) + &
